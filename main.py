...
class Board(Canvas):
    ...
    def __init__(self):
        super().__init__(
            ...
            background="lightyellow", highlightthickness=0
        )
    ...
    def createObjects(self):
        ...
        self.create_text(
            ...
            tag="score", fill="black", font='Times 10 bold'
        )
    ...
    def checkCollisions(self):
        ...
        for dot in dots:
            for over in overlap:
                if over == dot:
                    self.inGame = False
                    messagebox.showerror(title="Игра завершена!", message="Змейка столкнулась со своим хвостом")
                    sys.exit(1)

        if x1 < 0:
            self.inGame = False
            messagebox.showerror(title="Игра завершена!", message="Змейка столкнулась с границей поля")
            sys.exit(1)
